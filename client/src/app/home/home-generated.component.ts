...
export class HomeGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  grid0LoadData(event: any) {
    this.pnld.getReembolsosChartLists(`${event.filter == '' ? '' : event.filter + ' and '} ColaboradorEmail eq '${this.security.user.name}'`, event.top, event.skip, `${event.orderby}`, event.top != null && event.skip != null, ``, null, null)
    .subscribe((result: any) => {
      this.getReembolsosChartListsResult = result.value;
    }, (result: any) => {
